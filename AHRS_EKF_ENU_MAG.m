...
for i=1:gap:data_num-gap
    ...
    Y_MAG(j,1) = mean(sensor_data(i:i+gap-1,7));
    X_MAG(j,1) = mean(sensor_data(i:i+gap-1,8));
...
for k=1:L
    ...
    Pk_ = Ak * Pk(:,:,k) * Ak' + Q; 
    hk = [2*(X_(2)*X_(4)-X_(1)*X_(3)) 2*(X_(1)*X_(2)+X_(3)*X_(4)) X_(1)^2+X_(4)^2-X_(2)^2-X_(3)^2 2*(X_(2)*X_(3)+X_(1)*X_(4))*by + 2*(X_(2)*X_(4)-X_(1)*X_(3))*bz (X_(1)^2-X_(2)^2+X_(3)^2-X_(4)^2)*by + 2*(X_(1)*X_(2) + X_(3)*X_(4))*bz 2*(X_(3)*X_(4)-X_(1)*X_(2))*by + (X_(1)^2-X_(2)^2-X_(3)^2+X_(4)^2)*bz];
